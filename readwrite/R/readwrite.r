...
#' @usage
#' readwrite(filepath, writepath, varname = TRUE, sheet = 1, skip = 0, write = FALSE, ...)
...
#' @param sheet In case of importing an excel file, it refers to the sheet index that is to be imported from the file. In case of exporting an excel file, it refers to the name of the sheet (in double quotes) that you would like to give.
...
#' @inheritDotParams xlsx::read.xlsx -sheetIndex -header -file
#' @inheritParams xlsx::read.xlsx
#' @inheritParams xlsx::write.xlsx
...
#'     sheet = "sheet name", write = TRUE)
#' }
#' @export
readwrite <- function(filepath, writepath, varname = TRUE, sheet = 1, skip = 0, write = FALSE, ...){
  if (!write){
    ...
    utils::read.csv(filepath, header = varname, skip = skip)
        ...
        xlsx::read.xlsx(filepath, sheetIndex = sheet, header = TRUE, ...)
